defmodule MingaEditor.Shell.Traditional.Chrome.TUITest do
  ...
  describe "Chrome.TUI.build/4" do
    ...
    test "renders visible file tab as inactive when the active agent tab is hidden" do
      ...
      {tb, active_workspace} = TabBar.add_workspace(tb, "Active")
      {tb, _} = TabBar.add_workspace(tb, "Research")
      {tb, _} = TabBar.add_workspace(tb, "Review")
      tb = TabBar.move_tab_to_workspace(tb, 1, active_workspace.id)
      tb = TabBar.move_tab_to_workspace(tb, file_tab.id, active_workspace.id)

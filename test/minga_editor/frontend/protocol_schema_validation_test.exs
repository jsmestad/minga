defmodule MingaEditor.Frontend.ProtocolSchemaValidationTest do
  ...
  describe "gui_completion command fields match schema" do
    test "visible completion encodes placement header and items per command_fields" do
      ...
      # selected_item_id(string8) + total_count(u32) + matched_count(u32) + incomplete(u8) + generation(u32).
      ...
      # completion_item: kind(u8) + label/detail + stable ID + source + match ranges.
      # The model defaults exercise the empty metadata encodings after the visible fields.
      assert rest ==
               <<1::8, 3::16, "foo", 3::16, "bar", 0::8, 0::16, 0::8, 3::16, "doc", 0::8, 0::32,
                 0::32, 0::8, 0::32>>
    end

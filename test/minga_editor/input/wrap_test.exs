defmodule MingaEditor.Input.WrapTest do
  use ExUnit.Case, async: true

  alias MingaEditor.Input.Wrap

  # ── wrap_line/2 ───────────────────────────────────────────────────────────

  describe "wrap_line/2" do
    test "empty string returns single empty entry" do
      assert Wrap.wrap_line("", 20) == [%MingaEditor.Input.VisualLine{text: "", col_offset: 0}]
    end

    test "short line fits in one row" do
      assert Wrap.wrap_line("hello", 20) == [
               %MingaEditor.Input.VisualLine{text: "hello", col_offset: 0}
             ]
    end

    test "line exactly at width returns single entry" do
      assert Wrap.wrap_line("12345", 5) == [
               %MingaEditor.Input.VisualLine{text: "12345", col_offset: 0}
             ]
    end
    ...
    test "width below minimum truncates" do
      ...
      assert result == [%MingaEditor.Input.VisualLine{text: "hel", col_offset: 0}]
    end

defmodule MingaEditor.Renderer.LineTest do
  @moduledoc """
  ...
  describe "wide character (CJK / emoji) rendering" do
    ...
    test "cursor on CJK character lands at correct display column" do
      ...
      # Gutter width for 1-line file is 6 (2 sign + 1 fold + 2 digits + 1 space);
      # '好' starts at display col 2 -> 6+2=8
      assert cursor_col == 8, "Expected cursor at display col 8, got #{cursor_col}"
    end
    ...
    test "visual selection of CJK characters highlights correct grapheme cells" do
      ...
      # The draw command starts at display col 6 (gutter width: 2 sign + 1 fold + 2 digits + 1 space).
      # '你','好','世' land at cells 6, 7, 8 with :reverse.
      selected_cells = Enum.slice(row, 6, 3)

      assert Enum.all?(selected_cells, fn cell -> :reverse in cell.attrs end),
             "Expected cells 6-8 (你好世) to have :reverse attribute"

      # '界' at cell 9 must NOT be selected
  ...
  describe "visual selection rendering" do
    # Gutter width for 1-line file: 6 (2 sign + 1 fold + 2 digits + 1 space)
    @sel_gutter_w 6

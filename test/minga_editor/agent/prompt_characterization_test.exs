defmodule MingaEditor.Agent.PromptCharacterizationTest do
  @moduledoc """
  Characterization tests for the agent prompt editing flow.

  These pin the observable behavior of the prompt so regressions are
  caught mechanically. The prompt is backed by a Buffer.Process and
  edited via the standard Mode FSM (same pipeline as file buffers).

  The prompt has three concerns:

  1. **Text storage** — `Buffer.Process` (gap buffer). All text mutations
     (insert, delete, newline) go through GenServer calls.

  2. **Vim editing** — The standard Mode FSM handles motions, operators,
     visual mode, text objects, undo/redo. Keys are routed through
     `dispatch_prompt_via_mode_fsm`, which swaps the active buffer to
     the prompt buffer and runs the key through the same pipeline file
     buffers use.

  3. **Domain behavior** — Enter submits, history recall (up/down),
     paste block collapsing, @-mention completion. These are handled
     by UIState and the agent command handlers.
  """

  use ExUnit.Case, async: true

  alias MingaEditor.Agent.UIState
  alias Minga.Buffer.Process, as: BufferProcess

  # ── Helpers ──────────────────────────────────────────────────────────────────

  defp new_panel do
    panel = UIState.new()
    MingaEditor.Agent.PromptBuffer.set_input_focused(panel, true)
  end

  defp with_text(text) do
    panel = new_panel()
    BufferProcess.replace_content(panel.panel.prompt_buffer, text)
    panel
  end

  # ── Prompt text lifecycle ────────────────────────────────────────────────────

  describe "prompt text lifecycle" do
    test "empty panel has empty prompt text" do
      panel = new_panel()
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == ""
    end

    test "insert_char accumulates text" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "h")
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "i")
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "hi"
    end

    test "insert_newline splits lines" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "a")
      panel = MingaEditor.Agent.PromptBuffer.insert_newline(panel)
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "b")
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "a\nb"
    end

    test "clear_input saves to history and empties" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "x")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == ""
      assert panel.panel.prompt_history == ["x"]
    end

    test "delete_char removes last character" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "a")
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "b")
      panel = MingaEditor.Agent.PromptBuffer.delete_char(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "a"
    end

    test "delete_char no-op at buffer start" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.delete_char(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == ""
    end
  end

  # ── History recall ───────────────────────────────────────────────────────────

  describe "history recall" do
    test "history_prev recalls saved entry" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "x")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      panel = MingaEditor.Agent.PromptBuffer.history_prev(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "x"
    end

    test "history_next returns to empty" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "x")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      panel = MingaEditor.Agent.PromptBuffer.history_prev(panel)
      panel = MingaEditor.Agent.PromptBuffer.history_next(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == ""
    end

    test "history round-trip through multiple entries" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "a")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "b")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)

      panel = MingaEditor.Agent.PromptBuffer.history_prev(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "b"
      panel = MingaEditor.Agent.PromptBuffer.history_prev(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "a"
      panel = MingaEditor.Agent.PromptBuffer.history_next(panel)
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "b"
    end

    test "editing resets history index" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "x")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      panel = MingaEditor.Agent.PromptBuffer.history_prev(panel)
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "y")
      assert panel.panel.history_index == -1
    end
  end

  # ── Paste block handling ─────────────────────────────────────────────────────

  describe "paste block handling" do
    test "short paste inserts directly" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_paste(panel, "short")
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "short"
      assert panel.panel.pasted_blocks == []
    end

    test "long paste creates collapsed block" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_paste(panel, "line1\nline2\nline3")
      assert Enum.count(panel.panel.pasted_blocks) == 1
      assert hd(panel.panel.pasted_blocks).text == "line1\nline2\nline3"
      # prompt_text substitutes placeholder
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "line1\nline2\nline3"
    end

    test "clear_input removes paste blocks" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_paste(panel, "a\nb\nc")
      panel = MingaEditor.Agent.PromptBuffer.clear_input(panel)
      assert panel.panel.pasted_blocks == []
    end
  end

  # ── Focus management ─────────────────────────────────────────────────────────

  describe "focus management" do
    test "focusing starts prompt buffer" do
      panel = UIState.new()
      panel = MingaEditor.Agent.PromptBuffer.set_input_focused(panel, true)
      assert panel.panel.input_focused
      assert is_pid(panel.panel.prompt_buffer)
    end

    test "unfocusing preserves buffer content" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "x")
      panel = MingaEditor.Agent.PromptBuffer.set_input_focused(panel, false)
      refute panel.panel.input_focused
      assert MingaEditor.Agent.PromptBuffer.prompt_text(panel) == "x"
    end
  end

  # ── Multi-line editing ───────────────────────────────────────────────────────

  describe "multi-line editing" do
    test "cursor moves between lines" do
      panel = new_panel()
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "a")
      panel = MingaEditor.Agent.PromptBuffer.insert_newline(panel)
      panel = MingaEditor.Agent.PromptBuffer.insert_char(panel, "b")

      result = MingaEditor.Agent.PromptBuffer.move_cursor_up(panel)
      refute result == :at_top
    end

    test "backspace joins lines" do
      panel = with_text("ab\ncd")
      BufferProcess.move_to(panel.panel.prompt_buffer, {1, 0})
      panel = MingaEditor.Agent.PromptBuffer.delete_char(panel)
      assert MingaEditor.Agent.PromptBuffer.input_lines(panel) == ["abcd"]
    end
  end
end

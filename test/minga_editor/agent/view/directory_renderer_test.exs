defmodule MingaEditor.Agent.View.DirectoryRendererTest do
  use ExUnit.Case, async: true

  alias MingaEditor.Agent.View.DirectoryRenderer
  ...
  describe "render/6" do
    ...
    test "renders file entries with file icon" do
      rect = {0, 0, 60, 20}
      draws = DirectoryRenderer.render(rect, ".", ["README.md"], 0, false, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
    ...
    test "renders directory entries with folder icon" do
      rect = {0, 0, 60, 20}
      draws = DirectoryRenderer.render(rect, ".", ["src/", "lib/"], 0, false, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
      assert Enum.any?(texts, &String.contains?(&1, "📁"))
    ...
    test "renders mixed files and directories" do
      ...
      rect = {0, 0, 60, 20}
      draws = DirectoryRenderer.render(rect, ".", entries, 0, false, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
    ...
    test "scrolling skips top entries" do
      ...
      rect = {0, 0, 60, 10}
      draws = DirectoryRenderer.render(rect, ".", entries, 5, false, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
    ...
    test "fills remaining rows when entries are short" do
      rect = {0, 0, 40, 10}
      draws = DirectoryRenderer.render(rect, ".", ["a.ex"], 0, false, default_theme())
    ...
    test "handles empty entry list" do
      rect = {0, 0, 60, 10}
      draws = DirectoryRenderer.render(rect, ".", [], 0, false, default_theme())
      assert [_ | _] = draws
    end
  end
end

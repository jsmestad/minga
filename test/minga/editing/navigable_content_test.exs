defmodule Minga.Editing.NavigableContentTest do
  use ExUnit.Case, async: true
  use ExUnitProperties

  alias Minga.Buffer.Document
  alias Minga.Editing.NavigableContent
  ...
  describe "Motion functions work through Readable delegation" do
    test "word_forward works on BufferSnapshot" do
      ...
      pos = Minga.Editing.Motion.word_forward(s, NavigableContent.cursor(s))
      assert pos == {0, 6}
    end

    test "word_backward works on BufferSnapshot" do
      s = snapshot("hello world foo", {0, 12})
      pos = Minga.Editing.Motion.word_backward(s, {0, 12})
      assert pos == {0, 6}
    end

    test "line_end works on BufferSnapshot" do
      s = snapshot("hello world", {0, 0})
      pos = Minga.Editing.Motion.line_end(s, {0, 0})
      assert pos == {0, 10}
    end

    test "paragraph_forward works across lines" do
      s = snapshot("line one\nline two\n\nline four", {0, 0})
      pos = Minga.Editing.Motion.paragraph_forward(s, {0, 0})
    ...
    test "compose motion + replace_range for a delete-word operation" do
      ...
      word_end = Minga.Editing.Motion.word_forward(s, cursor)
  ...
  describe "properties" do
    ...
    property "motion functions never crash on BufferSnapshot" do
      check all(text <- string(:printable, min_length: 1, max_length: 200)) do
        ...
        _pos1 = Minga.Editing.Motion.word_forward(s, cursor)
        _pos2 = Minga.Editing.Motion.word_backward(s, cursor)
        _pos3 = Minga.Editing.Motion.line_start(s, cursor)
        _pos4 = Minga.Editing.Motion.line_end(s, cursor)
        _pos5 = Minga.Editing.Motion.document_start(s)
        _pos6 = Minga.Editing.Motion.document_end(s)
      end
    end
  end
end

defmodule Minga.UI.Prompt.AgentGroupRenameTest do
  use ExUnit.Case, async: true

  alias Minga.Editor.State.Tab
  alias Minga.Editor.State.TabBar
  alias Minga.UI.Prompt.AgentGroupRename
  ...
  describe "on_submit/2" do
    test "renames the active agent group" do
      ...
      state = %{shell_state: %Minga.Shell.Traditional.State{tab_bar: tb, status_msg: ""}}

      new_state = AgentGroupRename.on_submit("Research Bot", state)
      assert TabBar.active_group(new_state.shell_state.tab_bar).label == "Research Bot"
      assert TabBar.active_group(new_state.shell_state.tab_bar).custom_name == true
    ...
    test "trims whitespace from name" do
      ...
      assert TabBar.active_group(new_state.shell_state.tab_bar).label == "My Space"
    end
  ...
  describe "on_cancel/1" do
    test "returns state unchanged" do
      state = %{
        shell_state: %Minga.Shell.Traditional.State{tab_bar: TabBar.new(Tab.new_file(1, "a.ex"))}
      }

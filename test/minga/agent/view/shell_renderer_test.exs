defmodule Minga.Agent.View.ShellRendererTest do
  ...
  describe "render/7" do
    ...
    test "renders running status with spinner" do
      rect = {0, 0, 60, 20}
      draws = ShellRenderer.render(rect, "ls", "", :running, 0, 3, default_theme())
    ...
    test "renders error status with cross" do
      rect = {0, 0, 60, 20}
      draws = ShellRenderer.render(rect, "false", "exit 1", :error, 0, 0, default_theme())
      header = Enum.find(draws, fn {r, _c, _t, _o} -> r == 0 end)
      {_, _, text, _} = header
      assert String.contains?(text, "✗")
    end
    ...
    test "renders output lines with line numbers" do
      output = "line one\nline two\nline three"
      rect = {0, 0, 60, 20}
      draws = ShellRenderer.render(rect, "cat", output, :done, 0, 0, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
    ...
    test "scrolling skips top lines" do
      ...
      rect = {0, 0, 60, 10}
      draws = ShellRenderer.render(rect, "seq", output, :done, 5, 0, default_theme())
      texts = Enum.map(draws, fn {_r, _c, text, _opts} -> text end)
    ...
    test "fills remaining rows when output is short" do
      rect = {0, 0, 40, 10}
      draws = ShellRenderer.render(rect, "echo", "hi", :done, 0, 0, default_theme())

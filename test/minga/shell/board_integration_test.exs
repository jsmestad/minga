defmodule Minga.Shell.BoardIntegrationTest do
  @moduledoc """
  ...
  describe "Board grid rendering" do
    test "Board shell renders card grid with You card" do
      ...
      assert state.shell == Minga.Shell.Board
      assert Minga.Shell.Board.State.card_count(state.shell_state) == 1
    ...
    test "toggle_board switches from Traditional to Board" do
      ...
      state = editor_state(ctx)
      assert state.shell == Minga.Shell.Traditional

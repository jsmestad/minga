defmodule MingaAgent.Tools.LspCodeActionsTest do
  ...
  describe "execute/3 with edits" do
    ...
    test "rejects a versioned edit for a closed file", %{tmp_dir: tmp_dir} do
      source_path = Path.join(tmp_dir, "source_versioned.ex")
      target_path = Path.join(tmp_dir, "closed_versioned.ex")
      File.write!(source_path, "source\n")
      File.write!(target_path, "old_name\n")
      source = start_supervised!({BufferProcess, file_path: source_path, content: "source\n"})

      action = %{
        "title" => "Versioned replacement",
        "edit" => versioned_workspace_edit(target_path, "new_name", 7)
      }
    ...
    test "empty closed-file edit does not write or change metadata", %{tmp_dir: tmp_dir} do
      ...
      action = %{"title" => "Empty edit", "edit" => empty_workspace_edit(target_path)}
      client = start_fake_client(%{"textDocument/codeAction" => {:ok, [action]}})
      SyncServer.put_clients(source, [client])

      assert {:ok, "Code action \"Empty edit\": no edits to apply"} =
               LspCodeActions.execute(source_path, 0, apply: 1)
    ...
    test "rejects a versioned empty edit when the open-buffer version does not match", %{
      ...
      action = %{
        "title" => "Empty version mismatch",
        "edit" => versioned_empty_workspace_edit(path, 2)
      }
    ...
    test "rejects a versioned empty edit for a closed file without touching metadata", %{
      ...
      action = %{
        "title" => "Closed empty version",
        "edit" => versioned_empty_workspace_edit(target_path, 7)
      }
    ...
    @tag :tmp_dir
    test "neutral empty document does not inflate mixed code-action counts", %{tmp_dir: tmp_dir} do
      ...
      action = %{
        "title" => "Mixed edit",
        "edit" => mixed_workspace_edit(source_path, target_path, "new_name")
      }

      client = start_fake_client(%{"textDocument/codeAction" => {:ok, [action]}})
      SyncServer.put_clients(source, [client])

      assert {:ok, "Applied \"Mixed edit\": 1 edits across 1 files"} =
               LspCodeActions.execute(source_path, 0, apply: 1)

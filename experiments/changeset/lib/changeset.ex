defmodule Changeset do
  @moduledoc """
  In-memory changesets with filesystem overlays.

  A changeset tracks edits to files across a project without modifying
  the original. Edits are held in memory and materialized into an overlay
  directory where Unix tools (grep, compilers, test runners) see a
  coherent view of the project with changes applied.

  Overlay files are hardlinked to the originals (zero disk cost, appear
  as regular files to all tools). Modified files replace the hardlink
  with new content. Build artifacts get an isolated `_build` via
  `MIX_BUILD_PATH` to prevent contaminating the real project.

  ## Usage

      {:ok, cs} = Changeset.create("/path/to/project")

      :ok = Changeset.write_file(cs, "lib/math.ex", new_content)
      :ok = Changeset.edit_file(cs, "lib/util.ex", "old_name", "new_name")

      # Unix tools see the changes through the overlay
      {output, 0} = Changeset.run(cs, "grep -r 'new_name' lib/")
      {output, 0} = Changeset.run(cs, "mix compile")

      # Undo the last edit to a specific file
      :ok = Changeset.undo(cs, "lib/util.ex")

      # Merge back (with three-way merge if the human edited too)
      :ok = Changeset.merge(cs)

  ## Budget system

      {:ok, cs} = Changeset.create("/path/to/project", budget: 3)

      # After each verification attempt
      {:ok, 1} = Changeset.record_attempt(cs)
      {:ok, 2} = Changeset.record_attempt(cs)
      {:ok, 3} = Changeset.record_attempt(cs)
      {:budget_exhausted, 3, 3} = Changeset.record_attempt(cs)
      # Caller should discard and start fresh
  """

  alias Changeset.Server
  ...
  If the human edited the same files since the changeset was created,
  ...
  @doc "Discards all changes and cleans up."

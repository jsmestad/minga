...
impl Terminal {
    pub fn open() -> io::Result<Self> {
        let width = env_u16("COLUMNS").unwrap_or(80);
        let height = env_u16("LINES").unwrap_or(24);
        let tty = tty_file()?;
        ...
        terminal.writer.write_all(b"\x1b[?1049h\x1b[?25h")?;
        terminal.flush()?;
        Ok(terminal)
    }
    ...
    pub fn finish(&mut self) -> io::Result<()> {
        if self.real_tty {
            self.writer.write_all(b"\x1b[0m\x1b[?25h\x1b[?1049l")?;
        }

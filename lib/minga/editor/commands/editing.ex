defmodule Minga.Editor.Commands.Editing do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :delete_before) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :delete_at) do
    BufferServer.delete_at(buf)
    state
  end

  # Normal-mode x: deletes count character(s) at cursor and yanks them into the register.
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:delete_chars_at, count})
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:delete_chars_before, count})
      when is_integer(count) and count > 0 do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :insert_newline) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:insert_char, char})
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :insert_line_below) do
    {line, _col} = BufferServer.cursor(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :insert_line_above) do
    {line, _col} = BufferServer.cursor(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :join_lines) do
    {line, _col} = BufferServer.cursor(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:replace_char, char}) do
    BufferServer.delete_at(buf)
    BufferServer.insert_char(buf, char)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :toggle_case) do
  ...
  def execute(
        %EditorState{workspace: %{vim: %{mode_state: %ReplaceState{} = ms}, buffers: %{active: buf}}} = state,
  ...
  def execute(
        %EditorState{workspace: %{vim: %{mode_state: %ReplaceState{original_chars: []}}}} = state,
        :replace_restore
      ),
      do: state
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :undo) do
    BufferServer.undo(buf)
    state
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :redo) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :paste_before) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :paste_after) do
    {text, reg_type, state} = Helpers.get_register(state)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :indent_line) do
    tw = tab_width(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :dedent_line) do
    {line, col} = BufferServer.cursor(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:indent_lines, n}) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:dedent_lines, n}) do
    {cursor_line, _} = BufferServer.cursor(buf)
    total = BufferServer.line_count(buf)
    end_line = min(cursor_line + n - 1, total - 1)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:indent_motion, motion}) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:dedent_motion, motion}) do
    gb = BufferServer.snapshot(buf)
    cursor = Document.cursor(gb)
    target = Helpers.resolve_motion(gb, cursor, motion)
    {cursor_line, _} = cursor
  ...
  def execute(
        %EditorState{workspace: %{vim: %{mode_state: %VisualState{} = ms}, buffers: %{active: buf}}} = state,
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:reindent_lines, n}) do
    {cursor_line, _} = BufferServer.cursor(buf)
    total = BufferServer.line_count(buf)
    end_line = min(cursor_line + n - 1, total - 1)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:reindent_motion, motion}) do
    gb = BufferServer.snapshot(buf)
    cursor = Document.cursor(gb)
    target = Helpers.resolve_motion(gb, cursor, motion)
    {cursor_line, _} = cursor
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :comment_line) when is_pid(buf) do
    {line, _col} = BufferServer.cursor(buf)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:comment_motion, motion}) when is_pid(buf) do

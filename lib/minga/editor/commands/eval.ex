defmodule Minga.Editor.Commands.Eval do
  ...
  defp log_to_messages(%EditorState{workspace: %{buffers: %{messages: nil}}} = state, _text), do: state

  defp log_to_messages(%EditorState{workspace: %{buffers: %{messages: buf}}} = state, text) do

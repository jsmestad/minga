defmodule Minga.Editor.Commands.Visual do
  ...
  def execute(
        %EditorState{workspace: %{buffers: %{active: buf}, vim: %{mode_state: %VisualState{} = ms}}} = state,
  ...
  def execute(
        %EditorState{workspace: %{buffers: %{active: buf}, vim: %{mode_state: %VisualState{} = ms} = vim}} = state,

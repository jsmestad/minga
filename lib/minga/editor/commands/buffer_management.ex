defmodule Minga.Editor.Commands.BufferManagement do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :save) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :force_save) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :reload) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :cycle_line_numbers) when is_pid(buf) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :toggle_wrap) when is_pid(buf) do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:goto_line, line_num}}) do
    target_line = max(0, line_num - 1)
    BufferServer.move_to(buf, {target_line, 0})
    state
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:set, :number}})
      when is_pid(buf) do
    BufferServer.set_option(buf, :line_numbers, :absolute)
    state
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:set, :nonumber}})
      when is_pid(buf) do
    BufferServer.set_option(buf, :line_numbers, :none)
    state
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:set, :relativenumber}})
      when is_pid(buf) do
    current = BufferServer.get_option(buf, :line_numbers)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:set, :wrap}})
      when is_pid(buf) do
    BufferServer.set_option(buf, :wrap, true)
    state
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:execute_ex_command, {:set, :nowrap}})
      when is_pid(buf) do
  ...
  def alternate_file(%EditorState{workspace: %{buffers: %{active: buf}}} = state) when is_pid(buf) do
  ...
  @spec switch_to_buffer(state(), non_neg_integer()) :: state()
  defp switch_to_buffer(%{tab_bar: %TabBar{} = tb, workspace: %{buffers: %{list: buffers}}} = state, idx) do
  ...
  defp next_buffer(%EditorState{workspace: %{buffers: %{list: [_, _ | _] = buffers, active_index: idx}}} = state) do
  ...
  defp prev_buffer(%EditorState{workspace: %{buffers: %{list: [_, _ | _] = buffers, active_index: idx}}} = state) do
  ...
  defp remove_current_buffer(
         %EditorState{workspace: %{buffers: %{list: [_ | _] = buffers, active_index: idx} = bs}} = state
       ) do
      ...
      case new_buffers do
        ...
        _ ->
          ...
          EditorState.update_workspace(state, fn ws ->
            %{ws | buffers: %{bs | list: new_buffers, active_index: new_idx, active: new_active}}
          end)
  ...
  defp show_tool_prompt_if_normal(%EditorState{workspace: %{vim: %{mode: :normal}}, tool_prompt_queue: pending} = state)
       when pending != [] do
  ...
  defp focus_popup_window(state, buffer_pid) do
    case find_popup_for_buffer(state, buffer_pid) do
      {:ok, popup_window_id} ->
        %{state | workspace: %{state.workspace | windows: %{state.workspace.windows | active: popup_window_id}}}
  ...
  defp create_fallback_buffer(state, bs) do
    case DynamicSupervisor.start_child(
      ...
      {:ok, new_buf} ->
        %{state | workspace: %{state.workspace | buffers: %{bs | list: [new_buf], active_index: 0, active: new_buf}}}
        |> EditorState.sync_active_window_buffer()

      {:error, _} ->
        %{state | workspace: %{state.workspace | buffers: %{bs | list: [], active_index: 0, active: nil}}}
    end
  end

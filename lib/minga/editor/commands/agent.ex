defmodule Minga.Editor.Commands.Agent do
  ...
  @doc "Switches the input to normal mode."
  @spec input_to_normal(state()) :: state()
  def input_to_normal(state) do
    update_agent(state, fn agent ->
      %{agent | panel: PanelState.set_input_mode(agent.panel, :normal)}
    end)
  end

  @doc "Switches the input to insert mode (at cursor)."
  @spec input_to_insert(state()) :: state()
  def input_to_insert(state) do
    update_agent(state, fn agent ->
      %{agent | panel: PanelState.set_input_mode(agent.panel, :insert)}
    end)
  end

  @doc "Switches to insert mode with cursor moved one position right (vim `a`)."
  @spec input_to_insert_after(state()) :: state()
  def input_to_insert_after(state) do
    update_agent(state, fn agent ->
      panel = agent.panel
      tf = TextField.move_right(panel.input)
      panel = %{panel | input: tf}
      %{agent | panel: PanelState.set_input_mode(panel, :insert)}
    end)
  end

  @doc "Switches to insert mode at end of line (vim `A`)."
  @spec input_to_insert_eol(state()) :: state()
  def input_to_insert_eol(state) do
    update_agent(state, fn agent ->
      panel = agent.panel
      tf = TextField.move_end(panel.input)
      panel = %{panel | input: tf}
      %{agent | panel: PanelState.set_input_mode(panel, :insert)}
    end)
  end

  @doc "Switches to insert mode at first non-blank of line (vim `I`)."
  @spec input_to_insert_bol(state()) :: state()
  def input_to_insert_bol(state) do
    update_agent(state, fn agent ->
      panel = agent.panel
      tf = TextField.move_home(panel.input)
      panel = %{panel | input: tf}
      %{agent | panel: PanelState.set_input_mode(panel, :insert)}
    end)
  end

  @doc "Opens a new line below and enters insert mode (vim `o`)."
  @spec input_open_below(state()) :: state()
  def input_open_below(state) do
    update_agent(state, fn agent ->
      panel = agent.panel
      tf = panel.input |> TextField.move_end() |> TextField.insert_newline()
      panel = %{panel | input: tf}
      %{agent | panel: PanelState.set_input_mode(panel, :insert)}
    end)
  end

  @doc "Opens a new line above and enters insert mode (vim `O`)."
  @spec input_open_above(state()) :: state()
  def input_open_above(state) do
    update_agent(state, fn agent ->
      panel = agent.panel
      tf = panel.input |> TextField.move_home() |> TextField.insert_newline() |> TextField.move_up()
      tf = if tf == :at_top, do: panel.input, else: tf
      panel = %{panel | input: tf}
      %{agent | panel: PanelState.set_input_mode(panel, :insert)}
    end)
  end

  @doc "Applies a motion to the input cursor."
  @spec input_motion(state(), atom()) :: state()
  def input_motion(state, motion) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      new_tf = apply_input_motion(tf, motion)
      %{agent | panel: %{agent.panel | input: new_tf}}
    end)
  end

  @spec apply_input_motion(TextField.t(), atom()) :: TextField.t()
  defp apply_input_motion(tf, :left), do: TextField.move_left(tf)
  defp apply_input_motion(tf, :right), do: TextField.move_right(tf)

  defp apply_input_motion(tf, :up) do
    case TextField.move_up(tf) do
      :at_top -> tf
      new_tf -> new_tf
    end
  end

  defp apply_input_motion(tf, :down) do
    case TextField.move_down(tf) do
      :at_bottom -> tf
      new_tf -> new_tf
    end
  end

  defp apply_input_motion(tf, :word_forward) do
    pos = Motion.word_forward(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  defp apply_input_motion(tf, :word_backward) do
    pos = Motion.word_backward(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  defp apply_input_motion(tf, :word_end) do
    pos = Motion.word_end(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  defp apply_input_motion(tf, :line_start) do
    pos = Motion.line_start(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  defp apply_input_motion(tf, :line_end) do
    pos = Motion.line_end(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  defp apply_input_motion(tf, :first_non_blank) do
    pos = Motion.first_non_blank(tf, tf.cursor)
    TextField.set_cursor(tf, pos)
  end

  @doc "Deletes the character under the cursor (vim `x`)."
  @spec input_delete_char(state()) :: state()
  def input_delete_char(state) do
    update_agent(state, fn agent ->
      tf = TextField.delete_forward(agent.panel.input)
      %{agent | panel: %{agent.panel | input: tf}}
    end)
  end

  @doc "Deletes the character before the cursor (vim `X`)."
  @spec input_delete_char_before(state()) :: state()
  def input_delete_char_before(state) do
    update_agent(state, fn agent ->
      tf = TextField.delete_backward(agent.panel.input)
      %{agent | panel: %{agent.panel | input: tf}}
    end)
  end

  @doc "Deletes from cursor to end of line (vim `D`)."
  @spec input_delete_to_eol(state()) :: state()
  def input_delete_to_eol(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      eol = Motion.line_end(tf, tf.cursor)
      # line_end returns last char position; we want to delete through it
      {line, col} = eol
      eol_byte = col + byte_size(String.at(TextField.line_at(tf, line) || "", 0) || "")
      {new_tf, deleted} = TextField.delete_range(tf, tf.cursor, {line, eol_byte})
      register = if deleted != "", do: deleted, else: agent.panel.input_register
      %{agent | panel: %{agent.panel | input: new_tf, input_register: register}}
    end)
  end

  @doc "Changes from cursor to end of line and enters insert (vim `C`)."
  @spec input_change_to_eol(state()) :: state()
  def input_change_to_eol(state) do
    state = input_delete_to_eol(state)
    input_to_insert(state)
  end

  @doc "Deletes the current line (vim `dd`)."
  @spec input_delete_line(state()) :: state()
  def input_delete_line(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      {line, _} = tf.cursor
      {new_tf, deleted} = TextField.delete_line(tf, line)
      %{agent | panel: %{agent.panel | input: new_tf, input_register: deleted}}
    end)
  end

  @doc "Deletes from cursor to start of next word (vim `dw`)."
  @spec input_delete_word(state()) :: state()
  def input_delete_word(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      target = Motion.word_forward(tf, tf.cursor)
      {new_tf, deleted} = TextField.delete_range(tf, tf.cursor, target)
      %{agent | panel: %{agent.panel | input: new_tf, input_register: deleted}}
    end)
  end

  @doc "Deletes from cursor to start of line (vim `d0`)."
  @spec input_delete_to_bol(state()) :: state()
  def input_delete_to_bol(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      {line, _} = tf.cursor
      {new_tf, deleted} = TextField.delete_range(tf, {line, 0}, tf.cursor)
      %{agent | panel: %{agent.panel | input: new_tf, input_register: deleted}}
    end)
  end

  @doc "Changes the current line (vim `cc`): clears line content, enters insert."
  @spec input_change_line(state()) :: state()
  def input_change_line(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      {line, _} = tf.cursor
      line_text = TextField.line_at(tf, line) || ""
      {new_tf, _} = TextField.delete_range(tf, {line, 0}, {line, byte_size(line_text)})
      panel = PanelState.set_input_mode(%{agent.panel | input: new_tf, input_register: line_text}, :insert)
      %{agent | panel: panel}
    end)
  end

  @doc "Changes from cursor to start of next word (vim `cw`)."
  @spec input_change_word(state()) :: state()
  def input_change_word(state) do
    state = input_delete_word(state)
    input_to_insert(state)
  end

  @doc "Yanks the current line (vim `yy`)."
  @spec input_yank_line(state()) :: state()
  def input_yank_line(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      {line, _} = tf.cursor
      text = TextField.line_at(tf, line) || ""
      %{agent | panel: %{agent.panel | input_register: text}}
    end)
  end

  @doc "Yanks from cursor to start of next word (vim `yw`)."
  @spec input_yank_word(state()) :: state()
  def input_yank_word(state) do
    update_agent(state, fn agent ->
      tf = agent.panel.input
      target = Motion.word_forward(tf, tf.cursor)
      text = TextField.get_range(tf, tf.cursor, target)
      %{agent | panel: %{agent.panel | input_register: text}}
    end)
  end

  @doc "Pastes register content after cursor (vim `p`)."
  @spec input_paste_after(state()) :: state()
  def input_paste_after(state) do
    update_agent(state, fn agent ->
      register = agent.panel.input_register
      tf = agent.panel.input |> TextField.move_right() |> TextField.insert_text(register)
      %{agent | panel: %{agent.panel | input: tf}}
    end)
  end

  @doc "Pastes register content before cursor (vim `P`)."
  @spec input_paste_before(state()) :: state()
  def input_paste_before(state) do
    update_agent(state, fn agent ->
      register = agent.panel.input_register
      tf = TextField.insert_text(agent.panel.input, register)
      %{agent | panel: %{agent.panel | input: tf}}
    end)
  end

  @doc "Undo (placeholder, not yet implemented)."
  @spec input_undo(state()) :: state()
  def input_undo(state), do: state

  @doc "Enters visual character mode."
  @spec input_visual(state()) :: state()
  def input_visual(state) do
    update_agent(state, fn agent ->
      %{agent | panel: PanelState.set_input_mode(agent.panel, :visual)}
    end)
  end

  @doc "Enters visual line mode."
  @spec input_visual_line(state()) :: state()
  def input_visual_line(state) do
    update_agent(state, fn agent ->
      %{agent | panel: PanelState.set_input_mode(agent.panel, :visual_line)}
    end)
  end

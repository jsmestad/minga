defmodule Minga.Editor.Commands.Formatting do
  ...
  def format_buffer(%EditorState{workspace: %{buffers: %{active: buf}}} = state) when is_pid(buf) do
  ...
  defp queue_and_show_prompt(%EditorState{workspace: %{vim: %{mode: :normal}}} = state, tool_name) do

defmodule Minga.Editor.Commands.Git do
  ...
  defp close_file_tree_if_open(%EditorState{workspace: %{file_tree: %{tree: nil}}} = state), do: state
  ...
  defp open_git_status_for_root(state, git_root) do
    case Repo.lookup(git_root) do
      ...
      repo_pid ->
        ...
        %{state | workspace: %{state.workspace | keymap_scope: :git_status}, git_status_panel: panel_data}
    end
  end
  ...
  defp with_git_buffer(%EditorState{workspace: %{buffers: %{active: buf}}} = state, fun)
       when is_pid(buf) do

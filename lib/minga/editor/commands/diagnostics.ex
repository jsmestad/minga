defmodule Minga.Editor.Commands.Diagnostics do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: nil}}} = state, _cmd), do: state

  def execute(state, :diagnostic_list) do
    PickerUI.open(state, DiagPickerSource)
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :next_diagnostic) do
    navigate(state, buf, &Diagnostics.next/2)
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, :prev_diagnostic) do

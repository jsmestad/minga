defmodule Minga.Editor.Commands.Operators do
  ...
  def execute(%{buf: %{buffer: buf}} = state, {:delete_motion, motion}) do
    Helpers.apply_operator_motion(buf, state, motion, :delete)
  end

  def execute(%{buf: %{buffer: buf}} = state, {:change_motion, motion}) do
    Helpers.apply_operator_motion(buf, state, motion, :delete)
  end

  def execute(%{buf: %{buffer: buf}} = state, {:yank_motion, motion}) do
    Helpers.apply_operator_motion(buf, state, motion, :yank)
  end

  # ── Line-wise operators (dd / yy / cc / S) ────────────────────────────────

  def execute(%{buf: %{buffer: buf}} = state, :delete_line) do
  ...
  def execute(%{buf: %{buffer: buf}} = state, :change_line) do
    {line, _col} = BufferServer.cursor(buf)
    {:ok, yanked} = BufferServer.clear_line(buf, line)
    Helpers.put_register(state, yanked <> "\n", :delete)
  end

  def execute(%{buf: %{buffer: buf}} = state, :yank_line) do
    {line, _col} = BufferServer.cursor(buf)
    yanked = BufferServer.get_lines_content(buf, line, line)
    Helpers.put_register(state, yanked <> "\n", :yank)
  end
  ...
  def execute(%{buf: %{buffer: buf}} = state, {:delete_text_object, modifier, spec})
      when is_pid(buf) do
    Helpers.apply_text_object(state, modifier, spec, :delete)
  end

  def execute(%{buf: %{buffer: buf}} = state, {:change_text_object, modifier, spec})
      when is_pid(buf) do
    Helpers.apply_text_object(state, modifier, spec, :delete)
  end

  def execute(%{buf: %{buffer: buf}} = state, {:yank_text_object, modifier, spec})
      when is_pid(buf) do

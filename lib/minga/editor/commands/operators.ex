defmodule Minga.Editor.Commands.Operators do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:delete_motion, motion}) do
    if read_only?(buf),
      do: read_only_msg(state),
      else: Helpers.apply_operator_motion(buf, state, motion, :delete)
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:change_motion, motion}) do
    if read_only?(buf),
      do: read_only_msg(state),
      else: Helpers.apply_operator_motion(buf, state, motion, :delete)
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:yank_motion, motion}) do
    Helpers.apply_operator_motion(buf, state, motion, :yank)
  end

  # ── Line-wise operators (dd / yy / cc / S) ────────────────────────────────

  def execute(%EditorState{workspace: %{buffers: %{active: _buf}}} = state, :delete_line) do
    execute(state, {:delete_lines_counted, 1})
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:delete_lines_counted, count})
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: _buf}}} = state, :change_line) do
    execute(state, {:change_lines_counted, 1})
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:change_lines_counted, count})
      when is_integer(count) and count >= 1 do
    if read_only?(buf) do
      read_only_msg(state)
    else
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: _buf}}} = state, :yank_line) do
    execute(state, {:yank_lines_counted, 1})
  end

  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:yank_lines_counted, count})
      when is_integer(count) and count >= 1 do
    {line, _col} = BufferServer.cursor(buf)
    total = BufferServer.line_count(buf)
    end_line = min(line + count - 1, total - 1)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:delete_text_object, modifier, spec})
      when is_pid(buf) do
    if read_only?(buf),
      do: read_only_msg(state),
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:change_text_object, modifier, spec})
      when is_pid(buf) do
    if read_only?(buf),
      do: read_only_msg(state),
      else: Helpers.apply_text_object(state, modifier, spec, :delete)
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}}} = state, {:yank_text_object, modifier, spec})
      when is_pid(buf) do

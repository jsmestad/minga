defmodule Minga.Editor.Commands.Marks do
  ...
  def execute(%EditorState{workspace: %{buffers: %{active: buf}, vim: %{marks: marks}}} = state, {:set_mark, char})
  ...
  def execute(
        %EditorState{workspace: %{buffers: %{active: buf}, vim: %{marks: marks}}} = state,
  ...
  def execute(
        %EditorState{workspace: %{buffers: %{active: buf}, vim: %{last_jump_pos: last_pos}}} = state,
    ...
    BufferServer.move_to(buf, target)
    %{state | workspace: %{state.workspace | vim: %{state.workspace.vim | last_jump_pos: current_pos}}}
  end

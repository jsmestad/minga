defmodule Minga.Editor.HighlightSync do
  ...
  defp send_parse_for_pid(state, buf_pid, language, opts) do
    ...
    state = %{state | workspace: %{state.workspace | highlight: %{hl2 | version: version, syntax_overrides: syntax_overrides}}}
    touch_buffer(state, buf_pid)
  end
  ...
  def ensure_buffer_id(%EditorState{workspace: %{buffers: %{active: nil}}} = state), do: {0, state}
  ...
  defp apply_evictions(state, evicted_ids, remaining_timestamps) do
    ...
    new_hl = compute_post_eviction_state(state.workspace.highlight, evicted_ids, remaining_timestamps)
  ...
  def get_active_highlight(%EditorState{workspace: %{buffers: %{active: nil}}}), do: Highlight.new()
  ...
  def put_active_highlight(%EditorState{workspace: %{buffers: %{active: nil}}} = state, _hl_data), do: state

  def put_active_highlight(%EditorState{workspace: %{highlight: hl, buffers: %{active: buf}}} = state, hl_data) do
    %{state | workspace: %{state.workspace | highlight: %{hl | highlights: Map.put(hl.highlights, buf, hl_data)}}}
  end
  ...
  def put_highlight(%EditorState{workspace: %{highlight: hl}} = state, buf_pid, hl_data) do
    %{state | workspace: %{state.workspace | highlight: %{hl | highlights: Map.put(hl.highlights, buf_pid, hl_data)}}}
  end

  # Updates the active buffer's highlight via a function.
  @spec update_active_highlight(EditorState.t(), (Highlight.t() -> Highlight.t())) ::
          EditorState.t()
  defp update_active_highlight(%EditorState{workspace: %{buffers: %{active: nil}}} = state, _fun), do: state

defmodule Minga.Editor.Editing do
  ...
  **Compound accessors** read deep into sub-structs:
  ...
  def in_leader?(%EditorState{workspace: %{vim: %{mode_state: ms}}}) when is_map_key(ms, :leader_node),
  ...
  def reset_active_register(%EditorState{} = state) do
    %{state | workspace: %{state.workspace | vim: %{state.workspace.vim | reg: Registers.reset_active(state.workspace.vim.reg)}}}
  end

defmodule Minga.Editor.Mouse do
  ...
  * Wheel left/right: shift viewport horizontally
  ...
  def handle(%EditorState{workspace: %{buffers: %{active: nil}}} = state, _row, _col, _button, _mods, _type, _cc),
    do: state
  ...
  def handle(%EditorState{workspace: %{buffers: %{active: buf}}} = state, _r, _c, :wheel_up, _m, :press, _cc) do
    total_lines = BufferServer.line_count(buf)
    lines = scroll_lines(state)
    vp = active_window_viewport(state)
    new_vp = scroll_viewport(vp, -lines, total_lines)
  ...
  def handle(
        %EditorState{workspace: %{mouse: %MouseState{resize_dragging: {:vertical, sep_pos}}}} = state,
        _row,
        col,
        :left,
        _mods,
  ...
  def handle(
        %EditorState{workspace: %{mouse: %MouseState{resize_dragging: {:horizontal, sep_pos}}}} = state,
        row,
        _col,
        :left,
        _mods,
  ...
  def handle(
        %EditorState{workspace: %{mouse: %MouseState{dragging: true, anchor: anchor, drag_click_count: dcc}}} = state,
        row,
        col,
        :left,
        _mods,
  ...
  def handle(
        %EditorState{workspace: %{mouse: %MouseState{resize_dragging: {_, _}}}} = state,
        _r,
        _c,
        :left,
        _m,
    ...
    %{state | workspace: %{state.workspace | mouse: MouseState.stop_resize(state.workspace.mouse)}}
  end

  def handle(
        %EditorState{workspace: %{mouse: %MouseState{dragging: true}, vim: %{mode: :visual}}} = state,
        _r,
        _c,
        :left,
        _m,
    ...
    state = %{state | workspace: %{state.workspace | mouse: MouseState.stop_drag(state.workspace.mouse)}}
  ...
  def handle(%EditorState{workspace: %{mouse: %MouseState{dragging: true}}} = state, _r, _c, :left, _m, :release, _cc) do
  ...
  def handle(state, row, col, :none, _mods, :motion, _cc) do
    ...
    %{state | workspace: %{state.workspace | mouse: MouseState.set_hover(state.workspace.mouse, row, col)}}
  end
      ...
      {line, buf_col} ->
        ...
        case word_boundaries_at(buf, line, buf_col) do
          {word_start, word_end} ->
            ...
            %{state | workspace: %{state.workspace | mouse: MouseState.start_drag(state.workspace.mouse, {line, word_start})}}
  ...
  defp handle_triple_click(state, row, _col) do
    case mouse_to_buffer_line(state, row) do
      ...
      line ->
        ...
        state = EditorState.transition_mode(state, :visual, visual_state)
        %{state | workspace: %{state.workspace | mouse: MouseState.start_drag(state.workspace.mouse, {line, 0})}}
    end
  end
      ...
      {target_line, target_col} ->
        ...
        anchor =
          case state.workspace.vim.mode do
            :visual ->
              state.workspace.vim.mode_state.visual_anchor
  ...
  defp maybe_start_separator_drag(%EditorState{workspace: %{windows: %{tree: nil}}} = state, _row, _col), do: state
  ...
  defp maybe_start_separator_drag(state, row, col) do
    ...
    case WindowTree.separator_at(state.workspace.windows.tree, screen, row, col) do
      {:ok, {dir, sep_pos}} ->
        %{state | workspace: %{state.workspace | mouse: MouseState.start_resize(state.workspace.mouse, dir, sep_pos)}}
      ...
      :not_modeline ->
        ...
        case mouse_to_buffer_pos(state, row, col) do
          ...
          {target_line, target_col} ->
            ...
            state = cancel_mode_for_mouse(state)
            state = EditorState.transition_mode(state, :normal)
            %{state | workspace: %{state.workspace | mouse: MouseState.start_drag(state.workspace.mouse, {target_line, target_col})}}
        end
    end
  end

  @spec maybe_focus_window_at(state(), non_neg_integer(), non_neg_integer()) :: state()
  defp maybe_focus_window_at(%EditorState{workspace: %{windows: %{tree: nil}}} = state, _row, _col), do: state
  ...
  defp gutter_width(%EditorState{workspace: %{buffers: %{active: buf}}}, total_lines) do
    ln_style =
  ...
  defp mouse_to_buffer_line(%EditorState{workspace: %{buffers: %{active: buf}}} = state, row) do
    layout = Layout.get(state)
  ...
  @spec mouse_to_buffer_pos_single(state(), non_neg_integer(), non_neg_integer()) ::
          {non_neg_integer(), non_neg_integer()} | nil
  defp mouse_to_buffer_pos_single(%EditorState{workspace: %{buffers: %{active: buf}}} = state, row, col) do
    layout = Layout.get(state)
  ...
  defp auto_copy_selection(
         %EditorState{workspace: %{vim: %{mode: :visual, mode_state: ms}, buffers: %{active: buf}}} = state
       )
       when is_pid(buf) do
  ...
  defp clamp_cursor_to_viewport(%EditorState{workspace: %{buffers: %{active: buf}}} = state, direction) do
    vp = active_window_viewport(state)
    {cursor_line, cursor_col} = BufferServer.cursor(buf)
  ...
  defp maybe_auto_scroll(%EditorState{workspace: %{buffers: %{active: buf}}} = state, row) when row <= 0 do
    vp = active_window_viewport(state)
    page_move(buf, vp, -1)
    state
  end

  defp maybe_auto_scroll(%EditorState{workspace: %{buffers: %{active: buf}}} = state, row) do
    vp = active_window_viewport(state)
  ...
  defp enter_visual_if_needed(%EditorState{workspace: %{vim: %{mode: :visual}}} = state, _anchor), do: state
  ...
  defp cancel_mode_for_mouse(%EditorState{workspace: %{vim: %{mode: :command}}} = state) do

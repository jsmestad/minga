defmodule Minga.Editor do
  ...
  def handle_info({:minga_input, {:key_press, codepoint, modifiers}}, %{picker: picker} = state)
    ...
    new_state =
      case PickerUI.handle_key(state, codepoint, modifiers) do
        {s, {:execute_command, cmd}} -> execute_command(s, cmd)
        s -> s
      end
  ...
  defp execute_command(state, {:dot_repeat, count}) do
    replay_last_change(state, count)
  end
  ...
  * `{:line, start_line, end_line}` — linewise selection
  ...
  # Interior line of multi-line selection.

defmodule Minga.Config.Options do
  ...
  @option_specs [
    ...
    {:show_gutter_separator, :boolean, false,
     "Whether to draw a separator between the gutter and buffer text. Off by default to match the flat neovim-style visual hierarchy; set true to restore the divider."},
    ...
    {:modeline_left_segments, :atom_list,
     [:mode, :workspace, :filename, :git, :agent, :background_agent],
    ...
    {:resident_store_max_lines, :non_neg_integer, 65_536,
     "Maximum buffer line count that still receives full-document row residence (glitch-free fast scrolling). Defaults to the reviewed 65,536-line ceiling; resident row counts and row-delta counts use u32 on the wire. Real exposure is bounded by :resident_store_max_bytes and :max_file_size. Set to 0 to disable full residence entirely; over-ceiling or over-byte buffers fall back to viewport-windowed emit. Wrapped and folded buffers always use the windowed path."},
    ...
    {:agent_transcript_resident_max_bytes, :pos_integer, 8_388_608,
     "Maximum encoded byte size of the resident agent-chat transcript (gui_agent_transcript 0x86). The semantic model builder selects a contiguous newest suffix above this cap, preserving every retained message whole and marking the stream truncated when older messages are omitted. A p95 code-heavy session is ~1-2 MB, well under the 8 MB default. Set higher only if very long sessions truncate; the legacy gui_agent_chat (0x78) tail is unaffected."},

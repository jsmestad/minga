defmodule Minga.Input.FileTreeHandler do
  ...
  def handle_key(
        %EditorState{workspace: %{keymap_scope: :file_tree, file_tree: %{tree: %FileTree{}, focused: true}}} = state,
        cp,
        mods
      ) do
  ...
  def handle_key(%EditorState{workspace: %{keymap_scope: :file_tree}} = state, _cp, _mods) do
    {:passthrough, state}
  end
  ...
  def handle_mouse(
        %EditorState{workspace: %{keymap_scope: :file_tree, file_tree: %{tree: %FileTree{} = tree}}} = state,
        row,
        col,
        button,
        _mods,
  ...
  defp delegate_to_mode_fsm_with_tree_buffer(
         %EditorState{workspace: %{file_tree: %{buffer: buf}}} = state,
         cp,
         mods
       )
       when is_pid(buf) do
    ...
    state =
      if state.workspace.vim.mode != :normal do
  ...
  defp sync_tree_cursor_from_buffer(%EditorState{workspace: %{file_tree: %{tree: tree}}} = state, buf) do

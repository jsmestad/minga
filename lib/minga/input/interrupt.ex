defmodule Minga.Input.Interrupt do
  ...
  defp maybe_reset_scope(%EditorState{workspace: %{keymap_scope: :editor}} = state, resets), do: {state, resets}

  defp maybe_reset_scope(%EditorState{workspace: %{keymap_scope: scope}} = state, resets) do
    {%{state | workspace: %{state.workspace | keymap_scope: :editor}}, ["scope #{scope} → :editor" | resets]}
  end

  @spec maybe_reset_mode(EditorState.t(), [String.t()]) :: {EditorState.t(), [String.t()]}
  defp maybe_reset_mode(%EditorState{workspace: %{vim: %{mode: mode}}} = state, resets) when mode != :normal do
    {EditorState.transition_mode(state, :normal), ["mode #{mode} → :normal" | resets]}
  end

  defp maybe_reset_mode(%EditorState{workspace: %{vim: vim}} = state, resets) do
    fresh_state = Mode.initial_state()

    if mode_state_dirty?(vim.mode_state, fresh_state) do
      new_vim = %{vim | mode_state: fresh_state}
      {%{state | workspace: %{state.workspace | vim: new_vim}}, ["mode state reset (pending sequence cleared)" | resets]}
    else
      {state, resets}
    end
  end
  ...
  defp maybe_close_conflict(%EditorState{workspace: %{pending_conflict: nil}} = state, resets), do: {state, resets}

  defp maybe_close_conflict(state, resets) do
    {%{state | workspace: %{state.workspace | pending_conflict: nil}}, ["conflict prompt dismissed" | resets]}
  end

  @spec maybe_close_completion(EditorState.t(), [String.t()]) :: {EditorState.t(), [String.t()]}
  defp maybe_close_completion(%EditorState{workspace: %{completion: nil}} = state, resets), do: {state, resets}

  defp maybe_close_completion(%EditorState{workspace: %{completion: %Completion{}}} = state, resets) do

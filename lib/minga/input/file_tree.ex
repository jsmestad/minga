defmodule Minga.Input.FileTree do
  ...
  defp do_handle(%{file_tree_buffer: buf} = state, cp, _mods) when cp in [?q, @escape] do
    ...
    {:tree_handled, %{state | file_tree: nil, file_tree_focused: false, file_tree_buffer: nil}}
  end
  ...
  defp delegate_to_mode_fsm(%{file_tree_buffer: buf} = state, cp, mods) when is_pid(buf) do

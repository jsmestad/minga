defmodule MingaEditor.Input.Scoped do
  ...
  defp do_handle_agent_key(state, cp, mods) do
    ...
    if cp == @space and not panel.input_focused and band(mods, @ctrl) == 0 do
      ...
      if Minga.Editing.in_leader?(state) and not panel.input_focused do
        {:passthrough, state}
      else
  ...
  # dispatch_vim_key extracted to MingaEditor.Input.AgentPanel.dispatch_vim_key/3.
  ...
  # MingaEditor.Input.ToolApproval and MingaEditor.Input.DiffReview modules.

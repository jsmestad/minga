...
defmodule MingaEditor.Commands.BufferManagement do
  ...
  def execute(state, {:execute_ex_command, {:edit, file_path}}) do
    ...
    |> open_file_in_active_workspace(file_path)
  end
  ...
  defp cleanup_reload_state(state) do
    state
    |> MingaEditor.FeatureStateWorkflow.drop_source(:config)
    |> MingaEditor.FeatureStateWorkflow.drop_extension_sources()
  end
  ...
  defp switch_tab_by_visible_index(state, tb, n) do
    case Enum.at(TabBar.visible_file_tabs(tb), n - 1) do
      %Tab{id: id} -> MingaEditor.TabWorkflow.switch(state, id)
      nil -> state
    end
  end
  ...
  defp existing_file_tab_in_workspace(state, tb, workspace_id, file_ref) do
    if active_buffer_matches_file_ref?(state, file_ref) do
      MingaEditor.Shell.Runtime.active_tab(state.shell_runtime)
    else
  ...
  defp switch_to_buffer(
    ...
    case find_tab_for_buffer(tb, target_buf) do
      ...
      tab_id -> MingaEditor.TabWorkflow.switch(state, tab_id)
    end
  end
  ...
  defp next_tab(%{shell_runtime: %{state: %{tab_bar: %TabBar{} = tb}}} = state) do
    ...
    if next_tb.active_id != tb.active_id do
      MingaEditor.TabWorkflow.switch(state, next_tb.active_id)
    else
      state
    end
  end
  ...
  defp prev_tab(%{shell_runtime: %{state: %{tab_bar: %TabBar{} = tb}}} = state) do
    ...
    if prev_tb.active_id != tb.active_id do
      MingaEditor.TabWorkflow.switch(state, prev_tb.active_id)
    else
      state
    end
  end
  ...
  @spec handle_project_view_workspace_session_down(
          state(),
          TabBar.t(),
          WorkspaceModel.t(),
          ProjectView.t(),
  ...
  defp handle_project_view_workspace_session_down(
         state,
         tb,
         workspace,
         _view,
    ...
    case project_view_changed_files(workspace) do
      ...
      {:ok, files} ->
        keep_workspace_after_session_down(state, tb, workspace, session_pid, files, nil, reason)
  ...
  defp keep_workspace_after_session_down(state, _tb, workspace, session_pid, files, error, reason) do
    ...
    state =
      if is_pid(session_pid) do
        clear_session_from_tabs(state, session_pid, :error)
      else
        state
      end
    ...
    review =
      case error do
        nil -> WorkspaceReview.mark_needs_review(workspace.review, files)
        _ -> WorkspaceReview.mark_needs_review(workspace.review, files, error)
      end
    ...
    tb =
      TabBar.accept_workspace(
        state.shell_runtime.state.tab_bar,
        updated_workspace
      )

    state = WorkspaceWorkflow.install_tab_bar(state, tb)

    msg =
      case {reason in [:normal, :shutdown], error} do
        {true, nil} -> "Agent session ended, workspace drafts need review"
        {true, _} -> "Agent session ended, workspace review needs attention"
        {false, nil} -> "Agent session crashed, workspace drafts need review"
  ...
  @spec diff_entries_to_file_refs(String.t(), [map()]) :: [ProjectFileRef.t()]
  ...
  @spec file_ref_from_diff_entry(String.t(), map()) :: [ProjectFileRef.t()]
  defp file_ref_from_diff_entry(project_root, %{path: path}) when is_binary(path) do
    case ProjectFileRef.from_path(project_root, path) do
  ...
  defp close_agent_tab_without_session(state, tb, workspace, _workspace_view) do
    case project_view_changed_files(workspace) do
      ...
      {:ok, files} ->
        keep_workspace_after_session_down(state, tb, workspace, nil, files, nil, :normal)
  ...
  defp close_tab_or_quit(state) do
    {state, active_tab} = Workflow.resolve_active_tab(state)
    ...
    case MingaEditor.Shell.Runtime.active_tab(state.shell_runtime) do
      %Tab{context: context} when is_map(context) ->
        if TabContext.empty?(context) do
          ...
          MingaEditor.TabWorkflow.restore_context(state, context)
        end

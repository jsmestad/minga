defmodule MingaEditor.Commands.Lsp do
  @moduledoc """
  ...
  def execute(state, :lsp_info) do
    ...
    case clients do
      [] ->
        MingaEditor.Shell.Traditional.NoticeWorkflow.publish(state, "No language servers running")
  ...
  def execute(state, :lsp_start) do
    ...
    case configs do
      [] ->
        MingaEditor.Shell.Traditional.NoticeWorkflow.publish(
          state,
          "No LSP server available for #{filetype}"
        )
  ...
  defp clients_and_keys_for_active(%{workspace: %{buffers: %{active: buf}}} = _state) do

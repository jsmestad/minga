defmodule MingaEditor.ChangeTracking do
  @moduledoc """
  ...
  which calls back into `MingaEditor.do_handle_key/3` for replay.
  """
  ...
  def maybe_record_change(state, old_mode, new_mode, commands, key) do
    ...
    if rec.replaying do
      ...
      Editing.set_change_recorder(state, rec)
    end
  end
  ...
  def replay_last_change(state, count) do
    ...
    case ChangeRecorder.get_last_change(rec) do
      ...
      keys ->
        ...
        state = Editing.set_change_recorder(state, rec)

        state =
          Enum.reduce(keys, state, fn {codepoint, modifiers}, acc ->
            MingaEditor.do_handle_key(acc, codepoint, modifiers)
          end)

        rec = ChangeRecorder.stop_replay(MingaEditor.Editing.change_recorder(state))
        MingaEditor.Editing.set_change_recorder(state, rec)
    end
  end

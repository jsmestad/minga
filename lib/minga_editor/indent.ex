defmodule MingaEditor.Indent do
  ...
  Tree-sitter is the source of truth when the active buffer is registered with the parser manager. The parser returns an indent level from the language's `indents.scm` query, and this module converts that level into spaces or tabs using the buffer's indentation options. Buffers without a tree-sitter grammar, unavailable parsers, and unsupported parser responses fall back to copy-indent.
  """
  ...
  @type request_indent_fun :: (pid(), non_neg_integer() -> integer() | nil)

  @typedoc "Options for tree-sitter indentation."
  @type compute_opt :: {:fallback, String.t()} | {:request_indent, request_indent_fun()}

  @doc """
  Computes the indentation string for an existing line.

  Tree-sitter is queried for `line_num` using the buffer PID. Otherwise, the result falls back to either the explicit `:fallback` option or copy-indent from the previous line.
  """
  ...
  defp compute_with_tree_sitter(buf, line_num, fallback, opts) do
    ...
    case request_indent_level(buf, line_num, request_indent) do
      level when is_integer(level) and level >= 0 -> whitespace_for_level(buf, level)
      _ -> fallback
    end
  end

  @spec request_indent_level(pid(), non_neg_integer(), request_indent_fun()) :: integer() | nil
  defp request_indent_level(buffer_pid, line_num, request_indent) do
    request_indent.(buffer_pid, line_num)
  rescue
    _ -> nil
  catch
    :exit, _ -> nil

defmodule MingaEditor.Shell.Traditional.ModalWorkflow do
  ...
  def open(state, variant, payload) do
  ...
  @spec transition(EditorState.t(), ModalOverlay.variant(), ModalOverlay.payload()) ::
          EditorState.t()
  def transition(state, variant, payload) do
    state
    |> suppress_lower_surfaces()
  ...
  def close(state), do: EditorState.update_shell_state(state, &ShellState.close_modal/1)

  @doc "Dismisses a canceled modal."
  @spec dismiss(EditorState.t()) :: EditorState.t()
  def dismiss(state), do: EditorState.update_shell_state(state, &ShellState.dismiss_modal/1)
  ...
  @spec update_completion(EditorState.t(), (Minga.Editing.Completion.t() ->
          ...
          EditorState.t()
  def update_completion(state, update),
  ...
  @spec put_completion_trigger(EditorState.t(), MingaEditor.CompletionTrigger.t()) ::
          EditorState.t()
  def put_completion_trigger(state, trigger) do

...
Run one command to verify Minga through the same macOS accessibility hierarchy that screen readers and desktop agents use:

```bash
scripts/test_macos_accessibility
```

The command builds an isolated Minga app with its embedded BEAM release, creates a temporary project and configuration, then runs the workflow once cold and once warm. The test opens synthetic Unicode fixtures, inspects two editor panes, activates a tab and pane through accessibility actions, filters and activates a nonselected picker result, dismisses a picker without activation, and verifies text, selection, insertion state, and keyboard focus. It does not attach to or terminate a normal `com.minga.editor` app.
...
- macOS Accessibility permission for the process that runs the UI test
...
Exit status `0` means both the cold and warm workflows passed. Exit status `1` means the shipping accessibility behavior or the test build failed. Exit status `2` means a required macOS GUI, tool, or Accessibility permission was unavailable. A missing prerequisite is a failed validation run, not a skipped or passing test.

Evidence is retained at `_build/macos-accessibility/<UTC timestamp>-<pid>/`. Each attempted `cold` and `warm` directory contains the Xcode result bundle, the test log, and total runner timing. A run that reaches the test body also records timings for each workflow operation. A failed workflow also includes:

- `failure.txt`, with the unmet condition
- `accessibility-tree.txt`, with a bounded accessibility-tree snapshot
- `failure.png`, with the screen at failure time
- `infrastructure-failure.txt`, only when a GUI or permission prerequisite is missing

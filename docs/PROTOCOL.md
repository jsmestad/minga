...
**Protocol version:** The schema carries a `protocol_version` integer (currently 14). The BEAM and every frontend compile against it and exchange it in the `ready` handshake. The BEAM rejects a frontend whose version does not match and sends an explicit `protocol_error` instead of streaming frames the frontend cannot decode. Version 14 retires the native Tool Manager panel wire surface and action slots while preserving the Tool Manager service and picker command path. See "Protocol Version Negotiation" below.

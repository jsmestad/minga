...
- **Wave 6 / Track B:** The plan listed 9 violations but the actual audit found additional ones: `Minga.LSP.SemanticTokens` referenced `MingaEditor.UI.Highlight.Span`, and `Minga.Editing.Comment` referenced `MingaEditor.UI.Highlight.InjectionRange`. These were invisible to the credo check because it only scans `alias`/`import` lines, not fully qualified struct references in code. Fixed alongside the listed 9.

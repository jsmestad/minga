...
Worked examples. Completion menu, notifications (dismiss and action), observatory rows, edit-timeline entries, the float popup, and picker rows are client-resolved semantic interactions. The Go TUI tracks click zones (`completion:item:`, `notification:action:`, `observatory:node:`, `timeline:entry:`) and emits the matching `gui_action`, while SwiftUI uses native hit-testing and accessible buttons. For picker results and Actions entries, the BEAM emits a bounded offer with an opaque generation and per-entry activation ID. The frontend returns that immutable pair unchanged. The BEAM rejects stale or wrong identities, then owns selection, confirmation, preview promotion, source callbacks, command dispatch, and failure behavior.

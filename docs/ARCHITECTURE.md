...
`Renderer.Server` also owns the fallback-font registry across frames. A render operation receives that immutable registry in `RenderPipeline.Input`, threads it through window composition and `Frontend.Emit.Context`, and returns the updated value with the pipeline output. `FontRegistry` alone allocates IDs and marks registrations as emitted. Emit writes pending `register_font` commands before any frame command can reference those IDs; recovery marks every allocated family pending again without changing its ID or order. No process-local or frontend-owned font allocation state participates in rendering.
...
- **Identity transforms** (file-tree, completion, and picker selection): the frontend stores a source-owned `{generation, item_id}` over the BEAM's committed item model. Resolve by ID after same-generation reorder or refilter. Discard on generation change or `retained_item_miss`. A selection-only echo may advance the BEAM's committed selection without resending the full list and without erasing a retained local preview.
...
A frontend hit-tests its own rendered content and emits an intent ("activate this completion identity", "notification N dismissed"), exactly the contract SwiftUI already uses (native hit-test to `gui_action`). One pipeline, one way of doing things, on every client. The BEAM does not re-derive what a click means on rendered content. It owns structure: which rect a surface occupies, which surface wins when rects overlap (stacking depends on editor state only the BEAM has), and containment, so a click that misses every interactive element of a placed surface is swallowed (`MingaEditor.Input.OverlaySink`) instead of falling through to the buffer underneath.

Worked examples. Completion menu, notifications (dismiss and action), observatory rows, edit-timeline entries, the float popup, and picker rows are client-resolved semantic interactions. The Go TUI tracks semantic click zones and emits the matching `gui_action`, while SwiftUI uses native hit-testing and accessible buttons. For completion, picker, and file-tree items, the source emits an opaque generation and per-item ID. The frontend returns that pair unchanged with the surface and intent. The BEAM rejects stale or wrong identities, then owns selection, confirmation, preview promotion, source callbacks, command dispatch, and failure behavior.

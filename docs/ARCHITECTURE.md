...
Extension stop and reload paths consult code leases before purging path, git, or generated plugin modules. If a callback module is still leased, unload is rejected with a user-visible warning instead of racing `:code.purge/1` against in-flight work. Agent-facing contributions are stored in source-owned registries (`MingaAgent.Hooks.Registry`, `MingaAgent.MCP.ServerRegistry`, `MingaAgent.Skills.Registry`, and `MingaEditor.Agent.SlashCommand.Registry`) so cleanup removes a whole extension source without rescanning manifests or leaving stale active-session state. Slash commands keep their existing command path validation and external process behavior; they do not need callback-module leases unless a future slash-command callback API is introduced.

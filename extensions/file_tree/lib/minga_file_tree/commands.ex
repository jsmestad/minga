defmodule MingaFileTree.Commands do
  @moduledoc """
  ...
  alias MingaFileTree.DropIntent
  alias MingaFileTree.Freshness, as: FileTreeFreshness
  ...
  def handle_sidebar_action(state, "close", _context), do: close(state)
  def handle_sidebar_action(state, "click", %{index: index}), do: state |> move_cursor(index) |> open_or_toggle()
  def handle_sidebar_action(state, "toggle_entry", %{index: index}), do: state |> move_cursor(index) |> open_or_toggle()
  def handle_sidebar_action(state, "open_in_split", %{index: index}), do: open_in_split(state, index)
  def handle_sidebar_action(state, "new_file", %{index: index}), do: state |> move_cursor(index) |> new_file()
  def handle_sidebar_action(state, "new_folder", %{index: index}), do: state |> move_cursor(index) |> new_folder()
  def handle_sidebar_action(state, "edit_confirm", %{text: text}), do: state |> update_editing_text(text) |> confirm_editing()
  def handle_sidebar_action(state, "edit_cancel", _context), do: cancel_editing(state)
  def handle_sidebar_action(state, "delete", %{index: index}), do: state |> move_cursor(index) |> delete()
  def handle_sidebar_action(state, "rename", %{index: index}), do: state |> move_cursor(index) |> rename()
  def handle_sidebar_action(state, "duplicate", %{index: index}), do: state |> move_cursor(index) |> duplicate()

  def handle_sidebar_action(state, "move", %{source_index: source_index, target_dir_index: target_dir_index}),
    do: move(state, source_index, target_dir_index)

  def handle_sidebar_action(state, "drop", %{intent: intent}), do: drop(state, normalize_drop_intent(intent))
  def handle_sidebar_action(state, "collapse_all", _context), do: collapse_all(state)
  def handle_sidebar_action(state, "refresh", _context), do: refresh(state)
  def handle_sidebar_action(state, _action, _context), do: state

  @spec move_cursor(state(), non_neg_integer()) :: state()
  defp move_cursor(state, index) do
    case file_tree_state(state).tree do
      nil -> state
      tree -> update_file_tree(state, &FileTreeState.set_tree(&1, FileTree.select(tree, index)))
    end
  end

  @spec update_editing_text(state(), String.t()) :: state()
  defp update_editing_text(state, text) when is_binary(text) do
    case file_tree_state(state).editing do
      nil -> state
      %{} -> set_file_tree(state, FileTreeState.update_editing_text(file_tree_state(state), text))
    end
  end

  @spec open_in_split(state(), non_neg_integer()) :: state()
  defp open_in_split(state, index) do
    state = move_cursor(state, index)
    ...
    case selected_entry(state) do
      %{dir?: false, path: path} ->
        ...
        |> BufferRegistry.open_file_by_path(path)
  ...
  @spec selected_entry(state()) :: FileTree.entry() | nil
  defp selected_entry(state) do
    case file_tree_state(state).tree do
      %FileTree{} = tree -> FileTree.selected_entry(tree)
      _tree -> nil
    end
  end

  @spec normalize_drop_intent(DropIntent.t() | map()) :: DropIntent.t()
  defp normalize_drop_intent(%DropIntent{} = intent), do: intent

  defp normalize_drop_intent(intent) when is_map(intent) do
    DropIntent.new(
      source_paths: Map.get(intent, :source_paths, []),
      target_index: Map.get(intent, :target_index, 0),
      target_id: Map.get(intent, :target_id, ""),
      target_path_hash: Map.get(intent, :target_path_hash, 0),
      target_path: Map.get(intent, :target_path, ""),
      target_dir?: Map.get(intent, :target_dir?, false),
      modifiers: Map.get(intent, :modifiers, 0)
    )
  end
  ...
  defp close_tree(state) do
    ...
    state
    |> update_file_tree(&FileTreeState.close/1)
  ...
  defp open(state) do
    ...
    state
    |> update_file_tree(&FileTreeState.open(&1, tree, buf))

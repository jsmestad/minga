defmodule MingaFileTree.FeatureTest do
  # Mutates global input/sidebar registries.
  use ExUnit.Case, async: false

  alias Minga.Project.FileTree
  alias MingaEditor.Extension.Sidebar
  alias MingaFileTree.Feature, as: FileTreeFeature
  ...
  setup do
    Sidebar.unregister_source(MingaFileTree.Feature.source())

    on_exit(fn ->
      Sidebar.unregister_source(MingaFileTree.Feature.source())
      Input.unregister_source(MingaFileTree.Feature.input_source())
    end)
  ...
  test "FileTree state is stored through feature-state accessors" do
    ...
    assert MingaEditor.Session.State.get_feature_state(workspace, MingaFileTree.Feature.source(), :file_tree) ==
             file_tree
  end
  ...
  test "FileTree dynamic handler uses an extension-owned source that can unregister safely" do
    ...
    before_count = Enum.count(before_handlers, &(&1 == MingaFileTree.Input.Handler))

    :ok = Input.register_handler(source, MingaFileTree.Input.Handler, priority: 50)
    handlers = Input.surface_handlers(%{editing_model: Minga.Editing.Model.Vim})

    assert MingaFileTree.Feature.input_source() == {:extension, :minga_file_tree}
    assert Enum.count(handlers, &(&1 == MingaFileTree.Input.Handler)) == before_count + 1

    :ok = Input.unregister_source(source)
    handlers = Input.surface_handlers(%{editing_model: Minga.Editing.Model.Vim})

    assert Enum.count(handlers, &(&1 == MingaFileTree.Input.Handler)) == before_count
  end
  ...
  test "layout uses FileTree sidebar registry visibility and width" do
    ...
    file_tree = %FileTreeState{} |> MingaFileTree.State.open(tree, nil)
    ...
    closed_file_tree = MingaFileTree.State.close(file_tree)
    state = EditorState.set_file_tree(state, closed_file_tree)
    :ok = MingaFileTree.Feature.sync_sidebar(closed_file_tree)
    assert LayoutTUI.compute(state).file_tree == nil
  end

  test "workspace replacement re-syncs the active FileTree sidebar" do
    state = base_state(cols: 80, rows: 24)
    open_tree = %FileTreeState{} |> MingaFileTree.State.open(FileTree.new(File.cwd!(), width: 24), nil)
    ...
    :ok = MingaFileTree.Feature.sync_sidebar(open_tree)
    assert %{id: "file_tree", visible?: true, preferred_width: 24} = Sidebar.get("file_tree")

    closed_workspace =
      MingaEditor.Session.State.set_file_tree(state.workspace, MingaFileTree.State.close(open_tree))

    _state = EditorState.set_workspace(state, closed_workspace)
    :ok = MingaFileTree.Feature.sync_sidebar(MingaFileTree.State.close(open_tree))
  ...
  test "dropping FileTree feature state is safe and toggle recreates it" do
    ...
    state = MingaFileTree.Commands.toggle(state)

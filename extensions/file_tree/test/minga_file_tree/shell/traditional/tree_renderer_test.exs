defmodule MingaFileTree.Shell.Traditional.TreeRendererTest do
  ...
  alias MingaFileTree.Shell.Traditional.TreeRenderer
  alias MingaFileTree.Shell.Traditional.TreeRenderer.RenderInput
  ...
  describe "production state extraction" do
    test "renders active and dirty row state from editor state", %{tmp_dir: tmp_dir} do
      ...
      state =
        ...
        |> EditorState.set_file_tree(
          MingaFileTree.State.open(%FileTreeState{}, FileTree.new(tmp_dir, width: 32), nil)
        )
    ...
    test "renders loading and error status from file tree state", %{tmp_dir: tmp_dir} do
      ...
      for {file_tree, expected} <- [
            {MingaFileTree.State.loading(file_tree), "Loading files"},
            {MingaFileTree.State.error(file_tree, :eacces), "permission denied"}
          ] do

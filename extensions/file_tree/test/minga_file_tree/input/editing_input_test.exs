defmodule MingaEditor.Input.FileTreeEditingInputTest do
  ...
  defp make_state(tmp_dir) do
    ...
    %EditorState{
      ...
      workspace:
        ...
        |> SessionState.set_file_tree(%FileTreeState{} |> MingaFileTree.State.open(tree, buf)),
  ...
  defp make_editing_state(tmp_dir, opts \\ []) do
    ...
    file_tree = MingaFileTree.State.start_editing(ft(state), index, type, text)
  ...
  describe "filter input" do
    test "/ enters filtering, printable keys narrow, Enter accepts, and Escape clears", %{
      ...
      file_tree = MingaFileTree.State.replace_tree(ft(state), tree)
      ...
      file_tree = MingaFileTree.State.start_filtering(ft(state))

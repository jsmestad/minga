defmodule MingaFileTree.CommandsDropTest do
  @moduledoc """
  Command-state tests for BEAM-owned file-tree drop handling from native GUI intents.

  Classification: these tests call `Commands.drop/2` directly on constructed editor state. They still verify filesystem-visible behavior, stale-target rejection, overwrite protection, and open-buffer retargeting without booting the full EditorCase input/render stack.
  """
  ...
  describe "[command-state] drop/2 filesystem effects" do
    test "copies an external file into a directory target", %{
      ...
      state = Commands.drop(state, intent)
    ...
    test "copies an external file to the parent directory when dropped onto a file", %{
      ...
      _state = Commands.drop(state, intent)

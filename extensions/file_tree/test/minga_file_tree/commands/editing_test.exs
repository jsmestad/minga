defmodule MingaFileTree.CommandsEditingTest do
  @moduledoc """
  ...
  alias MingaFileTree.Commands
  alias MingaFileTree.Input.Handler, as: FileTreeHandler
  ...
  describe "[command-state] new file (a)" do
    ...
    test "creates file on disk after confirming a typed name", %{
      ...
      state = Commands.confirm_editing(state)
  ...
  describe "[command-state] new folder (A)" do
    test "enters new-folder editing mode", %{tmp_dir: dir, events_registry: events_registry} do
      ...
      state = Commands.new_folder(state)
      ...
      state = %EditorState{
        ...
        workspace:
          ...
          |> SessionState.set_file_tree(
            MingaFileTree.State.open(%FileTreeState{}, FileTree.new(dir) |> FileTree.refresh(), nil)
          ),
  ...
  describe "[command-state] cancel editing" do
    test "Escape-equivalent command cancels without filesystem changes", %{
      ...
      state = Commands.cancel_editing(state)
    ...
    test "confirm with empty text cancels", %{tmp_dir: dir, events_registry: events_registry} do
      state = dir |> make_state(events_registry) |> Commands.new_file()
  ...
  defp make_state(dir, events_registry, active_buffer \\ nil) do
    ...
    workspace =
      ...
      |> SessionState.set_file_tree(MingaFileTree.State.open(%FileTreeState{}, tree, nil))
  ...
  defp replace_editing_text(%EditorState{} = state, text) when is_binary(text) do
    ft = MingaFileTree.State.update_editing_text(ft(state), text)
  ...
  defp replace_tree(%EditorState{} = state, %FileTree{} = tree) do
    ft = MingaFileTree.State.replace_tree(ft(state), tree)
    EditorState.set_file_tree(state, ft)
  end
end

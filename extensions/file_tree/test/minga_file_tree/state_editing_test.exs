defmodule MingaFileTree.StateEditingTest do
  @moduledoc """
  ...
  describe "editing?/1" do
    test "returns false when editing is nil" do
      assert MingaFileTree.State.editing?(%FileTreeState{}) == false
    end

    test "returns true when editing state exists" do
      ft = MingaFileTree.State.start_editing(%FileTreeState{}, 0, :new_file)
      assert MingaFileTree.State.editing?(ft) == true
    end
  end

  describe "start_editing/4" do
    test "sets index, type, and empty text for new file" do
      ft = MingaFileTree.State.start_editing(%FileTreeState{}, 3, :new_file)
    ...
    test "sets type correctly for new folder" do
      ft = MingaFileTree.State.start_editing(%FileTreeState{}, 1, :new_folder)
    ...
    test "pre-fills text and original_name for rename" do
      ft = MingaFileTree.State.start_editing(%FileTreeState{}, 2, :rename, "old.txt")
    ...
    test "handles unicode filename for rename" do
      ft = MingaFileTree.State.start_editing(%FileTreeState{}, 0, :rename, "日本語.txt")
  ...
  describe "update_editing_text/2" do
    test "replaces text when editing is active" do
      ft =
        %FileTreeState{}
        |> MingaFileTree.State.start_editing(0, :new_file)
        |> MingaFileTree.State.update_editing_text("new_name.ex")
    ...
    test "is a no-op when editing is nil" do
      ft = MingaFileTree.State.update_editing_text(%FileTreeState{}, "ignored")
    ...
    test "handles empty string (clearing text)" do
      ft =
        %FileTreeState{}
        |> MingaFileTree.State.start_editing(0, :new_file, "something")
        |> MingaFileTree.State.update_editing_text("")
    ...
    test "handles unicode text" do
      ft =
        %FileTreeState{}
        |> MingaFileTree.State.start_editing(0, :new_file)
        |> MingaFileTree.State.update_editing_text("café.txt")
  ...
  describe "cancel_editing/1" do
    test "clears editing back to nil" do
      ft =
        %FileTreeState{}
        |> MingaFileTree.State.start_editing(0, :new_file, "partial")
        |> MingaFileTree.State.cancel_editing()

      assert ft.editing == nil
    end

    test "is idempotent on nil editing" do
      ft = MingaFileTree.State.cancel_editing(%FileTreeState{})
      assert ft.editing == nil
    end
  end

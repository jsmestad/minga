defmodule MingaEditor.FileTreeIntegrationTest do
  ...
  """
  # Uses the bundled FileTree extension contribution registry, which is global during extension package tests.
  ...
  defp assert_tree_visible(ctx) do
    assert Enum.any?(screen_text(ctx), &String.contains?(&1, "File Tree"))
  end

  defp refute_tree_visible(ctx) do
    refute Enum.any?(screen_text(ctx), &String.contains?(&1, "File Tree"))
  end
end

...
fn queryLookup(comptime name: []const u8, comptime qtype: QueryType) ?[]const u8 {
    ...
    if (comptime std.mem.eql(u8, name, "bash")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "bash/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "c")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "c/highlights.scm"), else => null };
    ...
    if (comptime std.mem.eql(u8, name, "cpp")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "cpp/highlights.scm"), .injections => @embedFile(query_dir ++ "cpp/injections.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "css")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "css/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "dart")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "dart/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "diff")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "diff/highlights.scm"), else => null };
    ...
    if (comptime std.mem.eql(u8, name, "elixir")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "elixir/highlights.scm"), .injections => @embedFile(query_dir ++ "elixir/injections.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "erlang")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "erlang/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "gleam")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "gleam/highlights.scm"), .injections => @embedFile(query_dir ++ "gleam/injections.scm"), .locals => @embedFile(query_dir ++ "gleam/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "go")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "go/highlights.scm"), else => null };
    ...
    if (comptime std.mem.eql(u8, name, "haskell")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "haskell/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "hcl")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "hcl/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "html")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "html/highlights.scm"), .injections => @embedFile(query_dir ++ "html/injections.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "java")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "java/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "javascript")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "javascript/highlights.scm"), .injections => @embedFile(query_dir ++ "javascript/injections.scm"), .locals => @embedFile(query_dir ++ "javascript/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "json")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "json/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "kotlin")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "kotlin/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "lua")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "lua/highlights.scm"), .injections => @embedFile(query_dir ++ "lua/injections.scm"), .locals => @embedFile(query_dir ++ "lua/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "make")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "make/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "markdown")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "markdown/highlights.scm"), .injections => @embedFile(query_dir ++ "markdown/injections.scm"), else => null };
    ...
    if (comptime std.mem.eql(u8, name, "nix")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "nix/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "ocaml")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "ocaml/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "php")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "php/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "python")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "python/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "r")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "r/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "ruby")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "ruby/highlights.scm"), .locals => @embedFile(query_dir ++ "ruby/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "rust")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "rust/highlights.scm"), .injections => @embedFile(query_dir ++ "rust/injections.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "scala")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "scala/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "scss")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "scss/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "toml")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "toml/highlights.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "tsx")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "tsx/highlights.scm"), .locals => @embedFile(query_dir ++ "tsx/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "typescript")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "typescript/highlights.scm"), .locals => @embedFile(query_dir ++ "typescript/locals.scm"), else => null };
    if (comptime std.mem.eql(u8, name, "yaml")) return switch (qtype) { .highlights => @embedFile(query_dir ++ "yaml/highlights.scm"), else => null };

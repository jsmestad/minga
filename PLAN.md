...
Added `version` counter to `Minga.Buffer.State` (incremented on every
`mark_dirty` call, including undo/redo). Editor snapshots buffer version
before key handling and compares after — triggers reparse on any content
mutation regardless of mode. Covers `dd`, `p`, `x`, `>>`, `<<`, undo, redo,
and all other mutation paths.

#### ~~2. Step 0g: Render boundary byte→grapheme cleanup~~ ✅ DONE
Converted cursor byte_col → grapheme column at the render boundary in
`Renderer.render/1`. Viewport horizontal scroll, cursor placement, modeline
column display, and visual selection bounds all use grapheme-indexed columns.
10 tests covering multi-byte characters, emoji, and ASCII baseline.
